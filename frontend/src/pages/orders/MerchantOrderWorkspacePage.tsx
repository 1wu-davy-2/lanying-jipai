...
import { Avatar, Badge, Button, Card, Col, Descriptions, Drawer, Empty, Image, Input, List, Modal, Progress, Row, Segmented, Space, Statistic, Tabs, Tag, Timeline, Typography, message } from "antd";
...
function MediaPreview({ urls }: { urls: string[] }) {
  ...
  return <Image.PreviewGroup items={urls.filter((url) => !isVideo(url))}>
    <Space wrap size={[8, 8]}>
      {urls.map((url) => isVideo(url) ? <video key={url} src={url} controls preload="metadata" style={{ width: 112, height: 82, objectFit: "cover", borderRadius: 4, background: "#172026" }} /> : <Image key={url} width={112} height={82} style={{ objectFit: "cover", borderRadius: 4 }} src={url} fallback="/placeholder-image.svg" />)}
    </Space>
  </Image.PreviewGroup>;
}
...
function FulfillmentCard({ item, onOpen }: { item: OrderFulfillment; onOpen: (item: OrderFulfillment) => void }) {
  ...
  return <Card className="fulfillment-card" size="small" bordered>
    <div className="fulfillment-card-heading">
      <Space size={10}>
        <Badge count={slotLabel(item.slot_no)} color="#0f766e" overflowCount={99} showZero><Avatar size={42} src={talent?.avatar_url} icon={<UserOutlined />}>{talent?.nickname?.slice(0, 1)}</Avatar></Badge>
    ...
    <div className="fulfillment-card-facts"><span>返图版本 {latest?.version || 0}</span><span>{item.return_required ? "需返货" : "拍后自留"}</span><span>结算 ¥{item.commission_amount}</span></div>
...
function SubmissionReview({ fulfillment, onRefresh }: { fulfillment: OrderFulfillment; onRefresh: () => Promise<void> }) {
  const [saving, setSaving] = useState(false);
  const latest = fulfillment.submissions?.[0];
  if (!latest || latest.status !== "PENDING_REVIEW") return null;
  const review = (approved: boolean) => {
    ...
    Modal.confirm({ title: approved ? `通过 ${talentName(fulfillment)} 的返图？` : `要求 ${talentName(fulfillment)} 修改返图？`, content: approved ? "通过后将进入返货或结算流程。" : <div><Typography.Paragraph type="secondary">要求修改会保留当前版本，并允许达人重新提交。</Typography.Paragraph><Input.TextArea defaultValue={revisionReason} rows={3} maxLength={255} onChange={(event) => { revisionReason = event.target.value; }} /></div>, okText: "确认", cancelText: "取消", onOk: async () => {
      setSaving(true);
      try { await reviewFulfillmentSubmission(fulfillment.id, latest.id, { approved, reason: approved ? undefined : revisionReason.trim() || "请根据订单要求修改返图后重新提交" }); await onRefresh(); message.success(approved ? "返图审核已通过" : "已要求达人修改返图"); } catch (error) { message.error(error instanceof Error ? error.message : "审核失败"); } finally { setSaving(false); }
    } });
  };
  return <Space wrap><Button type="primary" size="small" loading={saving} onClick={() => review(true)} icon={<CheckCircleOutlined />}>通过返图</Button><Button size="small" loading={saving} onClick={() => review(false)}>要求修改</Button></Space>;
}

export function MerchantOrderWorkspacePage({ orderId }: { orderId: number }) {
  ...
  const filteredFulfillments = useMemo(() => statusFilter === "all" ? fulfillments : fulfillments.filter((item) => item.status === statusFilter), [fulfillments, statusFilter]);
  ...
  return <div className="merchant-order-workspace">
    ...
    {tab === "attention" && <Card className="workspace-list-card"><List dataSource={attention} locale={{ emptyText: "暂无待处理事项" }} renderItem={(item) => { const talent = fulfillmentTalent(item); return <List.Item actions={[<Button key="detail" type="primary" size="small" onClick={() => setSelected(item)}>处理</Button>]}><List.Item.Meta avatar={<Avatar src={talent?.avatar_url}>{talent?.nickname?.slice(0, 1)}</Avatar>} title={<Space><span>名额 {slotLabel(item.slot_no)} · {talentName(item)}</span><OrderStatusTag status={item.status} /></Space>} description={item.status === "SUBMITTED" ? "达人已提交返图，等待商家审核" : item.status === "RETURNED" ? "返货已寄回，等待商家验收" : item.status === "DISPUTED" ? "该履约实例存在争议" : "等待商家确认同款商品"} /></List.Item>; }} /></Card>}

    <Drawer title={selected ? `名额 ${slotLabel(selected.slot_no)} · ${talentName(selected)} 的履约详情` : "履约详情"} width={Math.min(620, window.innerWidth - 24)} open={Boolean(selected)} onClose={() => setSelected(null)} extra={selected && <OrderStatusTag status={selected.status} />}>
      {selected && <div className="fulfillment-drawer-content"><Card size="small" className="fulfillment-drawer-talent"><Space><Avatar size={52} src={drawerTalent?.avatar_url} icon={<UserOutlined />}>{drawerTalent?.nickname?.slice(0, 1)}</Avatar><div><Typography.Title level={4}>{talentName(selected)}</Typography.Title><Typography.Text type="secondary">{drawerTalent?.level || "达人"} · {drawerTalent?.verify_status === "verified" ? "已实名" : "未实名"}</Typography.Text></div></Space><Descriptions column={1} size="small" items={[{ key: "source", label: "商品处理", children: productSourceLabels[selected.product_source] }, { key: "shipping", label: "寄样物流", children: selected.ship_to_model_tracking_no ? `${selected.ship_to_model_company || ""} ${selected.ship_to_model_tracking_no}` : "暂无" }, { key: "return", label: "返货物流", children: selected.return_tracking_no ? `${selected.return_company || ""} ${selected.return_tracking_no}` : selected.return_required ? "待填写" : "无需返货" }, { key: "settlement", label: "预计结算", children: `¥${(Number(selected.commission_amount) + Number(selected.product_subsidy_amount || 0)).toFixed(2)}` }]} /></Card><Card size="small" title="返图版本" className="fulfillment-drawer-section">{selected.submissions?.length ? <List dataSource={[...(selected.submissions || [])].reverse()} renderItem={(submission: FulfillmentSubmission) => <List.Item actions={[submission.status === "PENDING_REVIEW" && <SubmissionReview key="review" fulfillment={selected} onRefresh={refresh} />]}><List.Item.Meta title={<Space><span>版本 {submission.version}</span><Tag color={submission.status === "APPROVED" ? "green" : submission.status === "REVISION_REQUIRED" ? "orange" : "gold"}>{submission.status === "APPROVED" ? "已通过" : submission.status === "REVISION_REQUIRED" ? "要求修改" : "待审核"}</Tag></Space>} description={<span>提交于 {formatDate(submission.submitted_at)}{submission.reviewed_at ? ` · 审核于 ${formatDate(submission.reviewed_at)}` : ""}</span>} /><div className="fulfillment-submission-body"><MediaPreview urls={submission.media_urls} />{submission.remark && <Typography.Paragraph type="secondary">备注：{submission.remark}</Typography.Paragraph>}{submission.review_reason && <Typography.Paragraph type="warning">审核意见：{submission.review_reason}</Typography.Paragraph>}</div></List.Item>} /> : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="达人尚未提交返图" />}</Card><Card size="small" title="履约时间线" className="fulfillment-drawer-section"><Timeline items={[{ dot: <ClockCircleOutlined />, children: `申请通过 ${formatDate(selected.claimed_at)}` }, { children: `寄样 ${formatDate(selected.shipped_at)}` }, { children: `提交返图 ${formatDate(selected.submitted_at)}` }, { children: `返货/完成 ${formatDate(selected.returned_at || selected.completed_at)}` }]} /></Card><Space wrap>{selected.status === "CLAIMED" && <Button type="primary" onClick={() => { setShipmentTarget(selected); setShippingCompany(""); setShippingTrackingNo(""); }}>填写寄样物流</Button>}{selected.status === "OWNED_PRODUCT_REVIEW" && <Button type="primary" onClick={() => Modal.confirm({ title: `通过 ${talentName(selected)} 的同款审核？`, content: "通过后该达人将进入拍摄阶段。", okText: "通过", cancelText: "取消", onOk: async () => { try { await reviewFulfillmentOwnedProduct(selected.id, { approved: true }); await refresh(); message.success("同款审核已通过"); } catch (error) { message.error(error instanceof Error ? error.message : "审核失败"); } } })}>通过同款审核</Button>}{selected.status === "RETURNED" && <Button type="primary" onClick={() => Modal.confirm({ title: `确认验收 ${talentName(selected)} 的返货？`, okText: "确认验收", cancelText: "取消", onOk: async () => { try { await acceptFulfillment(selected.id); await refresh(); message.success("已完成该达人验收"); } catch (error) { message.error(error instanceof Error ? error.message : "验收失败"); } } })}>确认验收并结算</Button>}</Space></div>}
    </Drawer>
    <Modal title={shipmentTarget ? `填写名额 ${slotLabel(shipmentTarget.slot_no)} 的寄样物流` : "填写寄样物流"} open={Boolean(shipmentTarget)} onCancel={() => setShipmentTarget(null)} okText="确认寄出" cancelText="取消" confirmLoading={shippingSaving} onOk={async () => { if (!shipmentTarget || !shippingCompany.trim() || !shippingTrackingNo.trim()) { message.warning("请填写物流公司和运单号"); return; } setShippingSaving(true); try { await shipFulfillment(shipmentTarget.id, { company: shippingCompany.trim(), tracking_no: shippingTrackingNo.trim() }); await refresh(); setShipmentTarget(null); message.success("寄样物流已记录"); } catch (error) { message.error(error instanceof Error ? error.message : "寄样失败"); } finally { setShippingSaving(false); } }}><Typography.Paragraph type="secondary">本操作只影响 {shipmentTarget ? `名额 ${slotLabel(shipmentTarget.slot_no)} · ${talentName(shipmentTarget)}` : "当前达人"}。</Typography.Paragraph><Input value={shippingCompany} onChange={(event) => setShippingCompany(event.target.value)} placeholder="物流公司" maxLength={50} style={{ marginBottom: 12 }} /><Input value={shippingTrackingNo} onChange={(event) => setShippingTrackingNo(event.target.value)} placeholder="运单号" maxLength={50} /></Modal>
  </div>;
}
