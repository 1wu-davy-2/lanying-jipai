...
export function OrderDetailPage({ role, orderId }: { role: UserRole; orderId: number }) {
  ...
  const submitAction = async (values: { tracking_no?: string; company?: string; reason?: string }) => {
    ...
    setSaving(true);
    try {
      if (action === "ship") await shipOrder(order.id, { tracking_no: values.tracking_no ?? "", company: values.company ?? "" });
      if (action === "submit") {
      ...
      if (action === "reject") await rejectOrder(order.id, values.reason ?? "");
  ...
  const canShip = isMerchant && order.status === "CLAIMED";
  const canReceive = role === "model" && order.status === "SHIPPED_TO_MODEL";
  const canSubmit = role === "model" && order.status === "IN_PROGRESS";
  const canAccept = isMerchant && order.status === "RETURNED";
  const canCancel = isMerchant && order.status === "PUBLISHED";
  const mediaUrls = [...order.sample_images, ...order.submitted_media];
  return <div className="order-detail">
    <div className="page-heading"><Button type="text" icon={<ArrowLeftOutlined />} aria-label="返回订单列表" onClick={() => navigate(`/${role}/orders`)} /><Typography.Title level={2}>订单详情</Typography.Title><OrderStatusTag status={order.status} /></div>
    <div className="order-detail-grid">
      <Card className="content-card" title={order.title} extra={<strong>¥{order.commission_amount}</strong>}>
        ...
        <Divider />
        <Space wrap>
          {canCancel && <Button danger onClick={() => runConfirm("撤回订单", () => cancelOrder(order.id))}>撤回订单</Button>}
          ...
          {canAccept && <Button type="primary" onClick={() => runConfirm("验收通过", () => acceptOrder(order.id))}>验收通过</Button>}
          {canAccept && <Button danger onClick={() => setAction("reject")}>发起争议</Button>}
        </Space>
      </Card>
      <Card className="content-card" title="状态流转"><Timeline items={order.logs.map((log) => ({ color: log.to_status === "COMPLETED" ? "green" : undefined, children: <div><strong>{orderStatusLabel(log.to_status)}</strong><div className="muted-text">{log.remark || "状态更新"} {log.created_at ? `· ${new Date(log.created_at).toLocaleString()}` : ""}</div></div> }))} /></Card>
    </div>
