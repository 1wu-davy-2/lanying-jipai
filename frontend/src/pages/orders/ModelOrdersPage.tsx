...
export function ModelOrdersPage() {
  ...
  return <div className="model-orders-page">
    ...
    <div className="model-mobile-order-list">
      {isLoading ? <Skeleton active paragraph={{ rows: 5 }} /> : orders.length > 0 ? <>{orders.map((order) => { const fulfillmentStatus = order.fulfillment_status || order.status; return <button type="button" className="model-mobile-order" key={order.fulfillment_id || order.id} onClick={() => navigate(order.fulfillment_id ? `/model/fulfillments/${order.fulfillment_id}` : `/model/orders/${order.id}`)}>
